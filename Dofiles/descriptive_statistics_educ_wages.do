...
Date created: 13th March. 2022

Description: This dofile provides descriptive statistics of the AK dataset, and genreates figures and tables to be used in the final report.

Last edited: 13th March 2022, E. M. Bennett 


Sections.
	1. Import Data, Clean and Create Definitions
	2. Generate Summary Statistics Tables 
	3. Generate Education Histograms
*/
...
gen tag=1 if cohort>30.00 & cohort<30.40
replace tag=0 if tag==.


gen wklywge30 = exp(lwklywge)
******************************************************************************
**						2. Generate Summary Statistics						**
******************************************************************************
sum educ lwklywge wklywge if tag==1
sum educ lwklywge wklywge if tag==0

preserve
keep if tag==1
outreg2 using $output/3039_descriptive_stats_table.doc, replace sum(log) keep(age educ enocent esocent lwklywge married midatl mt neweng pac census qob race smsa soatl state wnocent wsocent) label
restore

preserve
keep if tag==0
outreg2 using $output/4049_descriptive_stats_table.doc, replace sum(log) keep(age educ enocent esocent lwklywge married midatl mt neweng pac census qob race smsa soatl state wnocent wsocent) label
restore
******************************************************************************
**						3. Generate Education Histograms					**
******************************************************************************
histogram educ if tag==1, freq discrete  title("1930-1939")
graph save "$output/3039educhist.gph", replace
histogram educ if tag==0, freq discrete title("1940-1949")
graph save "$output/4049educhist.gph", replace
gr combine $output/3039educhist.gph $output/4049educhist.gph, col(2) title("Figure 1: Years of Education Completed")
graph export "$output/educ_descriptive_statistic_histogram.pdf", replace